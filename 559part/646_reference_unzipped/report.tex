\documentclass[10pt,twocolumn,letterpaper]{article}

\usepackage{cvpr}
\usepackage{times}
\usepackage{epsfig}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage[table]{xcolor}
\usepackage[breaklinks=true,bookmarks=false]{hyperref}

\cvprfinalcopy
\def\cvprPaperID{****}
\def\httilde{\mbox{\tt\raisebox{-.5ex}{\symbol{126}}}}
\setcounter{page}{1}
\definecolor{blue300}{HTML}{9FD2F5}

\newcommand{\figplaceholder}[2]{
\fbox{\begin{minipage}[c][#2][c]{#1}
\centering \vspace{0.3em}
\textbf{Figure placeholder}\\
\small Replace with exported PDF/SVG figure.\\
Do not use notebook or terminal screenshots.
\end{minipage}}
}

\newcommand{\flowbox}[2]{
\setlength{\fboxsep}{4pt}
\fbox{\begin{minipage}[t]{#1}
\vspace{0.25em}
#2
\vspace{0.25em}
\end{minipage}}
}

\newcommand{\refcard}[3]{
\begin{minipage}[t]{0.23\textwidth}
\centering
\includegraphics[height=0.82in]{#1}\\[-0.15em]
{\scriptsize \textbf{#2}\\#3}
\end{minipage}
}

\newcommand{\resultcase}[4]{
\begin{minipage}[t]{0.31\textwidth}
\centering
\includegraphics[width=0.92\linewidth]{#1}\\[0.15em]
{\scriptsize \textbf{#2}}\\[-0.1em]
{\scriptsize #3}\\[0.05em]
{\scriptsize \emph{#4}}
\end{minipage}
}

\begin{document}

\title{HearthGen: Knowledge-Graph-Augmented Retrieval for Hearthstone Card Art Generation}

\author{Yicheng Jin, Zhiheng Wang, Zicong Zhang\\
Rice University\\
{\tt\small GitHub: \url{https://github.com/TraceOnSnow/HearthstoneCardGenerator}}
}

\maketitle

\begin{abstract}
  Hearthstone custom-card design is an active community practice: players often start from a natural-language idea, a class fantasy, a mechanic, or a partially written card text, and then need artwork that looks like it
  belongs in the game. This task is not only a style-transfer problem. A convincing generated card image should reflect Hearthstone's visual style, the card's class identity, and the gameplay semantics implied by its
  mechanics. We present HearthGen, an end-to-end prototype for knowledge-graph-augmented Hearthstone card art generation. The system converts official card data into structured semantics, builds a semantic knowledge graph
  (KG), retrieves mechanically relevant reference cards, and uses the retrieved evidence to guide Stable Diffusion 1.5 with a Hearthstone LoRA adapter. We evaluate a $2\times2$ generation setting: base Stable Diffusion vs.
  LoRA, and text-only vs. reference-conditioned generation. We also compare KG retrieval against TF-IDF and CLIP nearest-neighbor baselines. Our results suggest that LoRA improves visual style, while KG retrieval provides
  more interpretable and mechanically relevant references than lexical or visual similarity alone.
  \end{abstract}

\section{Introduction}

Trading-card artwork generation is a constrained creative task. A generated Hearthstone card should match the game art style, but it should also express the card's class, mechanics, and design lineage. For example, a user request such as ``a defensive Control Warrior Rager meme card'' is not just an image prompt. It refers to the Magma Rager family, the traditional 3-mana 5/1 statline, and Warrior armor mechanics. A generic text-to-image model or a simple CLIP nearest-neighbor search may retrieve visually plausible images, but it has no explicit representation of these gameplay relations.

We build HearthGen, a retrieval-augmented generation pipeline for Hearthstone card art. The system starts from Hearthstone JSON data and converts each card into a structured semantic record containing identity fields, stats, actions, keywords, generated-card references, derived-card links, and a LoRA caption. From these records we construct a semantic KG. User requests are parsed into structured queries and matched against the KG to retrieve reference cards. The retrieved evidence is then used to guide card design and image generation with Stable Diffusion~\cite{rombach2022ldm} and a LoRA adapter~\cite{hu2021lora}.

Our contributions are:
\begin{itemize}
\item A structured semantic representation for Hearthstone cards that combines deterministic metadata, rule-based action extraction, and optional LLM enrichment.
\item A semantic KG retrieval pipeline that returns faceted evidence rather than a single flat ranked list.
\item A generation and evaluation workflow comparing SD text-only, LoRA text-only, and reference-conditioned generation using TF-IDF, CLIP~\cite{radford2021clip}, and KG retrieval.
\end{itemize}



\begin{figure*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{p{0.47\textwidth}p{0.47\textwidth}}
\rowcolor{blue300}
\multicolumn{2}{c}{\textbf{From natural language to structured card-art generation}}\\[0.25em]
\flowbox{0.45\textwidth}{
\textbf{Natural-language request}\\
``DIY a new Rager meme card inspired by Magma Rager's 3-mana 5/1 statline, but make it a defensive Control Warrior card that gains Armor.''
}
&
\flowbox{0.45\textwidth}{
\textbf{Structured KG query}\\
class: Warrior; type: Minion; action: gain\_armor; related cards: Magma Rager, Rager; constraints: 5/1 statline, defensive Control Warrior, simple meme card.
}
\\[0.75em]
\rowcolor{blue300}
\multicolumn{2}{c}{\textbf{Faceted KG evidence: family anchor + mechanic anchor + class identity}}\\[0.25em]
\multicolumn{2}{c}{
\refcard{figures/figure1/magma_rager.jpg}{Magma Rager}{family/statline anchor}
\hfill
\refcard{figures/figure1/hookfist_3000.jpg}{Hookfist-3000}{Warrior armor mechanic}
\hfill
\refcard{figures/figure1/armorsmith.jpg}{Armorsmith}{class identity}
\hfill
\refcard{figures/figure1/eternium_rover.jpg}{Eternium Rover}{defensive flavor}
}
\\[0.75em]
\rowcolor{blue300}
\multicolumn{2}{c}{\textbf{Card design and generation prompt}}\\[0.25em]
\flowbox{0.45\textwidth}{
\textbf{Generated card text}\\
\textit{Iron Rager}. 3 Mana Warrior Minion, 5/1.\\
Battlecry: Gain 3 Armor.\\
Flavor: Molten fury tempered by iron will.
}
&
\flowbox{0.45\textwidth}{
\textbf{LoRA caption}\\
Hearthstone card art, a burly elemental rager forged from molten rock and clad in iron armor, Warrior class, glowing embers and metallic plates.
}
\\[0.75em]
\rowcolor{blue300}
\multicolumn{2}{c}{\textbf{Generated artwork and final playable-card composition}}\\[0.25em]
\begin{minipage}[t]{0.47\textwidth}
\centering
\includegraphics[height=1.25in]{figures/figure1/iron_rager_generated.png}\\[-0.2em]
{\scriptsize LoRA + KG reference output}
\end{minipage}
&
\begin{minipage}[t]{0.47\textwidth}
\centering
\includegraphics[height=1.25in]{figures/figure1/iron_rager_real_card.png}\\[-0.2em]

{\scriptsize External HearthDIY composition}
\end{minipage}
\end{tabular}
\caption{\textbf{End-to-end HearthGen case study.} A vague community-style request is converted into structured semantics, matched against faceted KG evidence, turned into a playable card design and LoRA caption, and finally rendered as Hearthstone-style artwork.}
\label{fig:overview}
\end{figure*}


\section{Related Work}

Our work combines retrieval-augmented generation, knowledge graphs, and diffusion-based image synthesis. Retrieval-augmented generation (RAG) uses external evidence to condition generation rather than relying only on model parameters~\cite{lewis2020rag}. In our setting, the retrieval corpus is not text passages but structured card semantics and artwork references. Knowledge graphs provide an interpretable way to represent entity and relation structure; we use the graph to connect cards to classes, keywords, actions, resources, and generated-card references.

For image generation, we use latent diffusion models~\cite{rombach2022ldm}, which generate images in a compressed latent space, and LoRA~\cite{hu2021lora}, a parameter-efficient finetuning method. CLIP~\cite{radford2021clip} is used both as a retrieval baseline and for automatic prompt-alignment metrics. We also use DINOv2~\cite{oquab2023dinov2} or CLIP image embeddings as proxy style/reference similarity metrics. The card data is derived from Blizzard's Hearthstone API and community-maintained data sources, including HearthstoneJSON and HearthSim tooling~\cite{hearthstoneapi,hearthstonejson,hearthsim}.




\section{Method}

\subsection{Structured Card Semantics}

The input is Hearthstone card JSON. We construct one semantic record per card, including collectible and non-collectible derived cards. Deterministic fields are extracted directly from JSON and metadata maps: card type, class, set, rarity, spell school, minion type, mana cost, attack, health, keywords, and child card IDs. Rule-based text extractors identify common gameplay actions such as \texttt{deal\_damage}, \texttt{summon}, \texttt{draw}, \texttt{gain\_armor}, \texttt{freeze}, and \texttt{destroy}. Each action stores type, amount, target, resource, trigger, condition, and raw text phrase when available.

We optionally enrich this base representation using MiniMax-M2.7~\cite{minimaxm27}. The LLM is not the source of deterministic facts; it only fills fields that rules often miss, such as generated-card roles, implicit conditions, action groups, and concise semantic summaries. The same semantic record is used for two downstream artifacts: a LoRA caption for generation and graph triples for KG retrieval.


\subsection{Semantic Knowledge Graph}

The KG is built from structured semantics rather than directly from raw card text. It is close to a bipartite graph: card nodes connect to typed semantic attribute nodes. Important attribute node types include class, card type, keyword, action, target, resource, spell school, generated card name, and related card name. Edges include \texttt{HAS\_CLASS}, \texttt{HAS\_CARD\_TYPE}, \texttt{HAS\_KEYWORD}, \texttt{PERFORMS\_ACTION}, \texttt{TARGETS}, \texttt{AFFECTS\_RESOURCE}, \texttt{GENERATES\_CARD}, and \texttt{HAS\_CHILD\_CARD}. Visual tags are intentionally excluded from the KG facts because they are derived from text and should not be double-counted as gameplay evidence.

Natural-language requests are parsed into structured queries. A query contains fields such as class, card type, actions, resources, keywords, generated card names, and related card names. Retrieval first scores cards by matching query fields to KG nodes. However, a single flat ranking can over-rank repeated mechanic matches. Therefore, for card design we return a faceted evidence package: family/name anchors, mechanic matches, identity matches, and a diversified shortlist. This is important for requests such as a Rager meme card, where a lower-ranked Magma Rager anchor may be more useful than another high-scoring Warrior armor card.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/kg/health_drink_kg_neighborhood.pdf}
\caption{\textbf{Semantic KG neighborhood sampled from the real graph.} The card node ``Health'' Drink connects to typed semantic attributes such as class, card type, spell school, keyword, action, and target. User queries are mapped into the same attribute space, so retrieval can match gameplay semantics rather than only raw text or visual similarity.}
\label{fig:semantic_kg}
\end{figure*}

\subsection{Generation Pipeline}

For image generation we compare Stable Diffusion v1.5 text-only generation, LoRA text-only generation, and reference-conditioned generation. The LoRA adapter is trained on Hearthstone artwork captions. Reference conditioning is implemented with image-to-image generation. We found that using multiple strong references can create ``fusion'' artifacts; the preferred policy is one primary visual reference plus text-only semantic hints from the rest of the KG evidence.


\subsection{Generation Pipeline}

We evaluate four generation settings using two binary design choices. The first choice is the image model: base Stable Diffusion 1.5 or Stable Diffusion 1.5 with a Hearthstone LoRA adapter. The second choice is
conditioning: text-only generation or reference-conditioned generation. This gives four methods:
\begin{itemize}
\item \textbf{SD text-only}: base Stable Diffusion 1.5 using only the generated prompt.
\item \textbf{SD + reference}: base Stable Diffusion 1.5 with a retrieved reference image.
\item \textbf{LoRA text-only}: Stable Diffusion 1.5 with the Hearthstone LoRA adapter using only the prompt.
\item \textbf{LoRA + reference}: Stable Diffusion 1.5 with the Hearthstone LoRA adapter and a retrieved reference image.
\end{itemize}

The text prompt is derived from the structured semantics and LoRA caption. The reference image is selected from retrieved Hearthstone artworks. This setup lets us separately measure whether style finetuning helps and
whether retrieved references help.

\section{Experiments}

We evaluate two questions. First, does KG retrieval return more useful Hearthstone references than lexical or visual baselines? Second, do LoRA and KG references improve card artwork generation?

\subsection{Retrieval Evaluation}

We compare three retrieval methods: TF-IDF over captions, CLIP text-to-image nearest-neighbor retrieval, and semantic KG retrieval. The queries include explicit mechanics, such as a Warlock lifesteal damage spell, and harder implicit requests, such as cards that generate Solar Eclipse and Lunar Eclipse or a defensive Rager meme card. Retrieval outputs top-$k$ reference cards with explanations.

\subsection{Generation Evaluation}

For generation, we compare SD text-only, SD with references, LoRA text-only, and LoRA with KG references on the same prompts. We report automatic proxy metrics: CLIP prompt alignment, style similarity to Hearthstone references, and reference similarity. These metrics do not fully measure card-design correctness, so we also include qualitative side-by-side examples and failure analysis.

\begin{figure*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{ccc}
\resultcase{figures/figure2/rager.png}{Iron Rager}{3 Mana Warrior Minion, 5/1. Battlecry: Gain 3 Armor.}{Vague meme/family request}
&
\resultcase{figures/figure2/druid_eclipse.png}{Eclipse Weaver}{4 Mana Druid Minion, 3/5. Battlecry: Add Solar Eclipse and Lunar Eclipse to your hand.}{Implicit related-card request}
&
\resultcase{figures/figure2/paladin_recruits.png}{Silver Hand Commander}{5 Mana Paladin Minion, 4/5. Battlecry: Summon two 1/1 Silver Hand Recruits. Your Silver Hand Recruits have +1 Attack.}{Token/archetype request}
\\[0.9em]
\resultcase{figures/figure2/mage_freeze.png}{Frozen Tempest}{5 Mana Mage Spell. Deal 2 damage to all enemy minions. Freeze them until end of turn.}{Mechanic-first request}
&
\resultcase{figures/figure2/warrior_weapon.png}{Iron Oathbreaker}{3 Mana Warrior Weapon, 3/2. After your hero gains Armor, this weapon has +1 Attack until end of turn.}{Fully specified card}
&
\resultcase{figures/figure2/bronze_dragon.png}{Chrono-Bronze Drake}{6 Mana Neutral Dragon, 5/7. Battlecry: Discover a card. If you control a Mech, reduce its Cost by (2).}{Art-first request}
\end{tabular}
\caption{\textbf{LoRA + KG-reference qualitative outputs.} Six representative user prompts are shown as final generated artwork with the corresponding card description. The set intentionally mixes vague creative requests, implicit Hearthstone references, mechanic-first briefs, fully specified card text, and art-first requests.}
\label{fig:qualitative}
\end{figure*}

\begin{table}[t]
\centering
\small
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Align.}$\uparrow$ & \textbf{Style}$\uparrow$ & \textbf{Ref.}$\uparrow$ \\
\midrule
SD text-only & 0.296 & 0.750 & -- \\
SD + KG ref & 0.295 & 0.837 & 0.796 \\
LoRA text-only & 0.301 & 0.858 & -- \\
LoRA + KG ref & 0.297 & 0.861 & 0.830 \\
\bottomrule
\end{tabular}
\caption{Generation proxy metrics on 19 DIY prompts. Alignment is CLIP prompt-image similarity; Style is similarity to Hearthstone art references; Ref. is similarity to the selected KG reference image.}
\label{tab:generation}
\end{table}

\begin{table}[t]
\centering
\small
\begin{tabular}{lcc}
\toprule
\textbf{Retrieval} & \textbf{Rel.@5}$\uparrow$ & \textbf{Main failure mode} \\
\midrule
TF-IDF & 0.679 & literal keywords \\
CLIP & 0.456 & visual but not mechanic match \\
KG & 0.806 & repeated facet over-ranking \\
\bottomrule
\end{tabular}
\caption{Retrieval proxy evaluation on 19 DIY prompts. KG returns interpretable reasons and faceted evidence, which is useful even when a flat ranking is imperfect.}
\label{tab:retrieval}
\end{table}

\section{Results and Analysis}

Figure~\ref{fig:overview} should show the complete system in one glance. The key point is that the KG is not only a way to find images. It also returns design evidence. In the Rager example, the user request is parsed into a Warrior minion with armor gain and related cards Magma Rager/Rager. The KG retrieves both Warrior armor cards and Rager-family anchors. The card-design module then produces \textit{Iron Rager}, a 3-mana 5/1 Warrior Elemental with ``Battlecry: Gain 3 Armor.'' This illustrates why faceted evidence is preferable to a single score-sorted list.

Figure~\ref{fig:qualitative} should show the generation ablation. The expected pattern is that SD text-only images often follow the prompt but lack Hearthstone style; LoRA text-only improves style but may miss specific mechanics; reference-conditioned outputs better match the selected card theme. KG references are most useful when the prompt depends on gameplay semantics or generated-card relations rather than surface-level visual similarity.

Tables~\ref{tab:generation} and~\ref{tab:retrieval} provide quantitative evidence. The automatic generation metrics are proxy measures: CLIP alignment checks prompt-image compatibility, while style and reference similarity check image embedding proximity. These numbers should be interpreted together with the qualitative grids, because a high reference similarity can also indicate over-copying. We observed that overly strong image-to-image conditioning can create fusion artifacts, so our final design recommends using one primary reference and treating other KG hits as text-only semantic hints.

\section{Discussion}

The results support two separate conclusions. First, the Hearthstone LoRA improves visual style. Compared with base Stable Diffusion, LoRA outputs more often resemble Hearthstone card illustrations in color, composition,
and fantasy rendering style. Second, KG retrieval improves the semantic relevance of references. TF-IDF often retrieves literal keyword matches, and CLIP often retrieves visually plausible images, but neither method
explicitly represents gameplay mechanics. The KG can match a request to class, type, action, keyword, and related-card nodes, which makes the retrieved evidence easier to interpret and more useful for card design.

This advantage comes from the structure of Hearthstone itself. Cards are not unstructured text prompts. They have classes, types, costs, stats, keywords, mechanics, tokens, and generated cards. Our KG is therefore not an
arbitrary extra component; it mirrors the way card information is naturally organized. In practice, this is most helpful for requests with implicit design knowledge, such as a Rager meme card, a Silver Hand Recruit payoff,
or a card that references Solar Eclipse and Lunar Eclipse.

However, the system is still limited. Automatic image metrics such as CLIP similarity only approximate human judgment. A generated image may score well while still failing to communicate the intended mechanic. Reference-
conditioned generation can also overfit to the selected reference image. Future work should include larger human evaluation, better reference-conditioning methods, and a more complete interface for turning generated
artwork into final framed Hearthstone cards.

\section{Conclusion}

HearthGen is an end-to-end prototype for KG-augmented Hearthstone card art generation. The main contribution is not a new diffusion architecture, but a structured semantic KG that retrieves mechanically grounded and interpretable references for generation and card design. LoRA improves visual style, while KG retrieval improves the relevance of prompts and references. The current system is limited by small-scale evaluation and imperfect reference conditioning, but it demonstrates a practical path from raw card data and user requests to structured card semantics and generated Hearthstone-style artwork.

\noindent
{\bf Acknowledgments.} We acknowledge the use of AI coding assistants for implementation support, debugging, and drafting assistance. All experiments, analysis, and final system design decisions were reviewed by the project team.


{\small
\bibliographystyle{ieee}
\bibliography{egbib}
}

\end{document}
